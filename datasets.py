...
def CIFAR10(folder_path, one_hot, classes):
    ...
    train_set.data = torch.tensor(train_set.data.reshape(train_set.data.shape[0], -1), dtype = cTYPE)
    test_set.data = torch.tensor(test_set.data.reshape(test_set.data.shape[0], -1), dtype = cTYPE)
    # train_set.data = train_set.data / 255
    # test_set.data = test_set.data / 255

    train_set.targets = torch.tensor(train_set.targets, dtype = cTYPE) % classes
    test_set.targets = torch.tensor(test_set.targets, dtype = cTYPE) % classes
    
    if one_hot:
        train_set.targets = torch.nn.functional.one_hot(train_set.targets.long(), classes)
        test_set.targets = torch.nn.functional.one_hot(test_set.targets.long(), classes)
        
    print(TEXT.format("Data size", str(tuple(train_set.data.shape))))    
    return train_set.data, train_set.targets.to(cTYPE), \
        test_set.data, test_set.targets.to(cTYPE)
...
def Ethylene(window = 2, stride = 2):
    ...
    trainY = torch.zeros((len(range(window, n, stride)) + 
                          len(range(window, m, stride)), 4), dtype = cTYPE)
    j = 0
